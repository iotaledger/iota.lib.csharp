...
namespace Iota.Api.Standard.UnitTests
{
    public class SigningTest
    {
        ...
        public void ShouldCreateCorrectSignatures()
        {
            ...
            int[] signature = signing.SignatureFragment(subNormalizedHash, subKey);
            ...
            int[] signature2 = signing.SignatureFragment(subNormalizedHash, subKey);
        ...
        public void ShouldCreateKeyWithCorrectSize()
        {
            ...
            int[] key_01, key_02, key_03;

            key_01 = signing.Key(Converter.ToTrits(TEST_SEED), 5, 1); 
            key_02 = signing.Key(Converter.ToTrits(TEST_SEED), 5, 2);
            key_03 = signing.Key(Converter.ToTrits(TEST_SEED), 5, 3);

            Assert.Equal(Signing.KeyLength, key_01.Length);
            Assert.Equal(2 * Signing.KeyLength, key_02.Length);
            Assert.Equal(3 * Signing.KeyLength, key_03.Length);
        }
        ...
        public void ShouldValidateSignatures()
        {
            ...
            Assert.True(signing.ValidateSignatures(RemoveChecksum(SIXTH_ADDR), new[] {SIGNATURE1, SIGNATURE2}, RemoveChecksum(FIRST_ADDR)));
        }
