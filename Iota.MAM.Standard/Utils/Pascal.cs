...
namespace Iota.MAM.Utils
{
    public class Pascal
    {
        public static int[] Zero = {1, 0, 0, -1};

        public static Tuple<long, int> Decode(int[] trits)
        {
            // check Zero
            ulong index;
        ...
        public static void Encode(long input, int[] trits)
        {
            if (input == 0)
            {
            ...
            else
            {
                ...
                if (TritsHelper.Trits2Long(trits, length - Constants.TritsPerTryte, Constants.TritsPerTryte) < 0)
                {
                    ...
                    {
                        trits[length - Constants.TritsPerTryte + j] = -trits[length - Constants.TritsPerTryte + j];
                    }
                }

                int[] encodingTrits = new int[trits.Length - length];
        ...
        private static ulong End(int[] trits, int index)
        {
