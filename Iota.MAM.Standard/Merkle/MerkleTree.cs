...
namespace Iota.MAM.Merkle
{
    ...
    {
        private static readonly int[] NullHash = new int[Constants.HashLength];

        public static MerkleNode CreateMerkleTree(
            int[] seed, int index, uint count,
            int security,
            ICurl c1,
            ICurl c2,
            ICurl c3)
        ...
        // root
        public static int Root(int[] address, int[] hashes, int index, ICurl curl)
        {
            int i = 1;
            int numBeforeEnd = 0;
            int[] outTrits = new int[Constants.HashLength];
            ...
            ICurl c1, ICurl c2, ICurl c3)
        {
            ...
            int[] hash = new int[Constants.HashLength];
        ...
        private static MerkleNode CreateMerkleLeaf(
            ...
            ICurl c1, ICurl c2, ICurl c3)
        {
            int[] hash = ComputeHash(seed,index,security,c1,c2,c3);
        ...
        private static int[] ComputeHash(
            ...
            ICurl c1, ICurl c2, ICurl c3)
        {
            int[] subseed = HashHelper.Subseed(seed, index, c1);

            c1.Reset();

            //iss::subseed_to_digest(&subseed, security, &mut hash, c1, c2, c3);
            int[] digest = HashHelper.SubseedToDigest(subseed, security, c1, c2, c3);
            ...
            int[] address = HashHelper.Address(digest, c1);
